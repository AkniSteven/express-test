...
export const submitTransaction = async ({
  ...
  const RELAYER_ACCOUNT: string | undefined = 'testrest.near';
  ...
  try {
    ...
    const delegateForSubmission = signedDelegate(delegate);

    console.log('TEST SIGNED DELEGATE')
    console.log(delegateForSubmission);
    console.log(delegateForSubmission.signedDelegate.delegateAction.actions)


    const result = await relayerAccount.signAndSendTransaction({
      actions: [delegateForSubmission],
